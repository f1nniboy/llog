...
export class ConfigError extends AIError<ConfigErrorData> {
    ...
    public static simple(issue: ConfigErrorIssue) {
        return new ConfigError({
            issues: [ issue ]
        });
    }
    ...
    public toString(): string {
        ...
        builder.push(`${chalk.bold(this.options.data.issues.length)} ${this.options.data.issues.length > 1 ? "issues" : "issue"} found:\n`);

        builder.push(
            ...this.options.data.issues.map(i => `- ${this.issueToString(i)}\n`)
        );
    ...
    private issueToString(issue: ConfigErrorIssue): string {
        ...
        if (issue.expected && issue.got) {
            if (issue.got == "undefined") builder.push(`missing ${chalk.bold(issue.expected)}`)
            else builder.push(`expected ${chalk.bold(issue.expected)}, got ${chalk.bold(issue.got)}`);
        }
