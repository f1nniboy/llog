    ...
    public static adjacentKeys(rows: string[]): Record<string, string[]> {
        ...
        for (let r = 0; r < rows.length; r++) {
            ...
            for (let i = 0; i < row.length; i++) {
                ...
                if (r > 0 && i < rows[r - 1].length) adjacent.push(rows[r - 1][i]);

                // Add the character below if it exists
                if (r < rows.length - 1 && i < rows[r + 1].length) adjacent.push(rows[r + 1][i]);
    ...
    public static add(str: string): string {
        ...
        if (str.length === 0) return str;
            ...
            if (!adjKeys) return str;
                ...
                let newChar = adjKeys[Math.floor(Math.random() * adjKeys.length)];
