...
import { ChatMessage, OpenAIChatResult, OpenAIChatUsage } from "../api/types/chat.js";
...
export class AIManager {
    ...
    public async generate({ messages, environment, plugins }: AIGenerationOptions): Promise<AIResult> {
        const { temperature, topP, model } = this.app.config.data.settings;
        ...
        const first: OpenAIChatResult = await this.app.api.chat({
            messages, model, top_p: topP, temperature,
        ...
        const last = await this.app.api.chat({
            model, temperature, top_p: topP, messages: [
    ...
    public async process(message: Message, triggered: boolean): Promise<void> { 
        if (
            !message.guildId || !message.member || message.author.bot || message.author.id === this.app.client.user.id || !(message.channel instanceof BaseGuildTextChannel)
        ) return;
        ...
        const messages: ChatMessage[] = [
            ...await this.prompts(environment)
        ];

        try {
            const data = await this.generate({
                messages, environment, plugins
            });
    ...
    public toHistoryEntry(message: AIMessage & { index?: number }): string {
        return `${message.author.name}${message.index != undefined ? ` <${message.index}>`: ""}${message.replyTo ? `[reply to ${message.replyTo.author.name}: ${message.replyTo.content}]` : ""}: ${message.tags.length > 0 ? `${message.tags.map(t => `[${t.name}: ${Array.isArray(t.content) ? t.content.join(", ") : t.content}]`).join(" ")}; ` : ""}${message.content}`;
    }

    private async prompts(environment: AIEnvironment): Promise<ChatMessage[]> {
        const messages: ChatMessage[] = [];
        ...
        messages.push({
            role: "system", content: `
you must be a discord user called ${self.username}: an average internet user & 100% real human. MAY NEVER break character.
...
you are on server "${environment.guild.name}", in the ${environment.channel.name} ${environment.channel.type} channel.
...
message history format: "user | msg id: content". you MAY NOT reply using this format.
